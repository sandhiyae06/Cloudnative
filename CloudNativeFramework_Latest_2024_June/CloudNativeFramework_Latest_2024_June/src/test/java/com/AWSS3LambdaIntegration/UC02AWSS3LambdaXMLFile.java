...
public class UC02AWSS3LambdaXMLFile extends BaseTest_TestNG {
	...
	public  void xmlFileS3LambdaIntegration() throws Exception {
		...
		test.log(Status.INFO, "Uploading file ***"+globalProp.getProperty("UC02sourcefilename")+"***to bucket ***"+sourceBucketName+"*** in AWS S3");
		resuableComponents.uploadFiletoS3(sourceBucketName, s3client, globalProp.getProperty("XMLFilePath"),globalProp.getProperty("UC02sourcefilename"));
		test.log(Status.INFO, "Validating file ***"+globalProp.getProperty("UC02sourcefilename")+"*** in AWS S3 bucket ***"+sourceBucketName+"***");
		resuableComponents.ValidateUploadedFileinS3(sourceBucketName, s3client, globalProp.getProperty("UC02sourcefilename"), test);
		
		
		
		test.log(Status.INFO, "Moving file ***"+globalProp.getProperty("UC02sourcefilename")+"***to bucket ***"+destinationBucketName+"*** in AWS S3");
		resuableComponents.moveFilebetweenS3(sourceBucketName, destinationBucketName, s3client, globalProp,globalProp.getProperty("UC02sourcefilename"), globalProp.getProperty("UC02sourcefilename"));
		test.log(Status.INFO, "Validating file ***"+globalProp.getProperty("UC02sourcefilename")+"*** in AWS S3 bucket ***"+destinationBucketName+"***");
		test.log(Status.INFO, "Validating the updated file ***"+globalProp.getProperty("UC02sourcefilename")+"*** in AsourceBucketNameWS S3 bucket ***"+destinationBucketName+"***");
		resuableComponents.ValidateUploadedFileinS3(destinationBucketName, s3client, globalProp.getProperty("UC02sourcefilename"), test);
		test.log(Status.INFO, "*********************Download file from AWS S3 - Starts***********************");
		
		resuableComponents.downloadFileFromS3(destinationBucketName, s3client, globalProp.getProperty("UC02sourcefilename"),globalProp.getProperty("DownloadedXMLFilePath"));
		resuableComponents.downloadFileFromS3(destinationBucketName, s3client,globalProp.getProperty("UC02targetfilename"),globalProp.getProperty("DownloadedXMLFilePath") );
		
		BufferedReader srcBuffReader = comparatorResuableComponents
				.loadXMLFile(globalProp.getProperty("DownloadedXMLFilePath")+ "\\"+globalProp.getProperty("UC02sourcefilename"));
		BufferedReader tgtBuffReader = comparatorResuableComponents
				.loadXMLFile(globalProp.getProperty("DownloadedXMLFilePath")+ "\\"+globalProp.getProperty("UC02targetfilename"));
